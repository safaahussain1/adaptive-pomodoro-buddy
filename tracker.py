...
def teardown(session, noise_detector, cap):
    ...
    if session.history_logs:
        ...
        summary_context = {
            ...
            "elapsed_mins": (
                (session.history_logs[-1]["timestamp"] - session.session_start_time) / 60
            ),
            "noise_level_db": (
                sum(p.get("noise_db", 40) for p in session.history_logs) / len(session.history_logs)
            ),
            "top_app": max(
...
def run_study_buddy():
    ...
    LLM_DISPLAY_SECONDS = 10.0

    print("=" * 60)
    print("  Adaptive Pomodoro Buddy — Sensing Matrix Online")
    print("  Press [Q] or [Esc] to quit and save the session report.")
    print("=" * 60)
            ...
            while True:
                ...
                if current_time - last_log_time >= 1.0:
                    ...
                    session.log_frame_state(
                        gaze_status, posture_status, blink_counter, noise_db=cached_noise_db
                    )
                    blink_counter = 0
                    ...
                    if decision:
                        ...
                        context = {
                            ...
                            "blink_rate_per_min": (
                                sum(p["blinks_in_last_sec"] for p in session.history_logs[-60:])
                                * 60
                                / max(len(session.history_logs[-60:]), 1)
                            ),
                        }
                        threading.Thread(
                ...
                h, w = frame.shape[:2]
                header = frame.copy()
                cv2.rectangle(header, (0, 0), (w, 155), (0, 0, 0), -1)
                cv2.addWeighted(header, 0.5, frame, 0.5, 0, frame)
                ...
                display_app = (
                    ...
                    else cached_current_app
                )

                cv2.putText(
                    frame, f"[{block_label}] {mins:02d}:{secs:02d}",
                    (20, 42), cv2.FONT_HERSHEY_DUPLEX, 1.1, timer_color, 2,
                )
                cv2.putText(
                    frame, f"Focus: {recent_focus:.0f}%",
                    (20, 82), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 0), 2,
                )
                cv2.putText(
                    frame, f"App: {display_app}",
                    (20, 117), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (255, 128, 0), 2,
                )
                cv2.putText(
                    frame,
                    f"Noise: {noise_label} ({cached_noise_db:.0f} dB)  |  Posture: {posture_status}",
                    (20, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.52, (200, 200, 200), 1,
                )
                ...
                if llm_overlay.get("message") and msg_age < LLM_DISPLAY_SECONDS:
                    ...
                    cv2.putText(
                        frame, " ".join(line1),
                        (15, h - 58), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1,
                    )
                    if line2:
                        cv2.putText(
                            frame, " ".join(line2),
                            (15, h - 28), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1,
                        )
